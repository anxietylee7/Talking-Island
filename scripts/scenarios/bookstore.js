...
window.BOOKSTORE_SCENARIO = {
  ...
  truth: {
    summary:
      '야미는 어젯밤 밤톨 서점에 예약해 둔 책을 픽업하러 갔다. 카운터에는 ' +
      '아무도 없었고, 야미는 장부에 사인한 뒤 책을 집어 들었다. 그 과정에서 ' +
        ...
        effects: [
          ...
          { type: 'injectNpcContext', npcId: 'bamtol',
            text: '너는 방금 서점을 둘러보다 책 한 권이 비어 있는 자리를 ' +
                  '발견했다. 어제까지 분명히 꽂혀 있던 자리다. 누가 사갔나? ' +
                  '이상하다 생각만 한다. 굳이 장부까지 확인할 건 아닌 것 같고, ' +
                  '생각만 하는 중. 대화 톤은 평소보다 짧고 미간을 찌푸린 상태.' },
          // [Wave 1] 증거 목격 후 NPC 선발화.
        ...
        effects: [
          ...
          { type: 'injectNpcContext', npcId: 'bamtol',
            text: '너는 방금 사진관에서 차카의 사진을 봤다. 야미가 서점 안에 ' +
                  '있는 모습이 찍혀있었다. 차카는 "야미가 책을 많이 사더라"라고 ' +
                  '했는데, 어제 야미가 픽업해가는 책은 1권뿐이었다. 차카의 말을 ' +
                  '들은 너는 야미가 훔쳤다는 생각이 든다. 장부는 아직 확인하지 ' +
                  '않았다. 지금 감정이 앞서고 있고, 이 길로 야미를 찾아가 따질 ' +
                  '생각이다.' },
    ...
    quest_active: [
...

      {
        ...
        effects: [
          ...
          // [Wave 1 / 이슈 H 해결] 유저가 야미 접근 시 퀘스트만 생성되고 대화창은 빈 채로 열려
          // 유저가 상황을 인지 못 하던 문제. 야미의 situation 대사를 chatHistory 에 미리
          // 삽입해 대화창 열자마자 UI 에 노출되도록.
          //
          // 주의: 아래 텍스트는 quests.yami_dream_crisis.branches[0].situation 과 사실상 같은
          //       내용이어야 일관됨. 현재 단일 분기라 가능. 분기 늘어나면 엔진 쪽에서
          //       situation 을 chatHistory 에 자동 주입하는 기능 필요 (향후 과제).
          { type: 'npcSpeaksFirst', npcId: 'yami', emotion: 'sad',
            text: '...너 잠깐 와볼래?\n\n' +
                  '오늘 밤톨 사장님한테 다시 가봤어. 독서 모임 장소 빌리려고. ' +
                  '근데 얼굴도 안 보시더라. 책 훔친 사람한테 모임 자리를 내줄 리 없지. ' +
                  '내가 장부에 사인한 것까지 다 있는데, 안 들어주셔.\n\n' +
                  '너한텐 말하고 싶었어. 넌 내 얘길 들어줄 것 같아서.' },
        ],
      },
    ...
    dormant: [
      {
        ...
        primaryNpcId: 'yami',
        // [4차] 2차 truth 와 정합성 재작성.
        // 이전 scene: "옆에 있던 책이 떨어졌다" — 지나치게 직접적.
        // 새 scene:   책 떨어지는 사실을 유지하되, "미끄러져 책장 아래로" 표현.
        scene:
          '야미가 서점 뒷문을 열고 들어가 예약해 둔 책을 꺼낸다. ' +
          '책을 집어 드는 순간 옆의 책 한 권이 흔들려 책장 아래로 미끄러져 들어간다. ' +
          '야미는 알아채지 못한 채 장부에 또렷이 사인을 남기고 서점을 나선다.',
        // publicSummary: 플레이어 리포트에 표시되는 "목격자 시점의 공개 사실".
    ...
    triggered: [
      {
        ...
        scene:
          ...
          '예약 책 한 권만 들어 있다. 야미가 문을 두드려 해명하려 하지만, ' +
          '밤톨은 문을 열지 않는다.',
  ...
  quests: {
    yami_dream_crisis: {
      ...
      branches: [
        {
          key: 'default',
          condition: { type: 'default' }, // 항상 이 분기 선택
          situation:
            '야미가 조용히 다가와요. 손에는 구겨진 포스터가 그대로 들려 있어요.\n\n' +
            '"...오늘 밤톨 사장님한테 다시 가봤어. 독서 모임 장소 빌리려고. ' +
            '근데 얼굴도 안 보시더라. 책 훔친 사람한테 모임 자리를 내줄 리 없지. ' +
            '내가 장부에 사인한 것까지 다 있는데, 안 들어주셔."\n\n' +
            '야미가 당신을 한참 쳐다봐요.\n\n' +
      ...
      // 해결 시 효과
      onResolved: [
        { type: 'setFlag', key: 'bookstore_ending_route' }, // 엔딩 분기 시점에 밤톨 호감도 읽음
      ],
      ...
      milestones: [
        ...
        {
          // ID 유지, 내용 교체. 2차 서사: 밤톨이 장부를 확인했지만 "느낌"을 우선함.
          // → 유저가 할 일은 "장부라는 증거의 무게"를 환기시키거나
          //   밤톨이 자기 원칙(정확함)을 배반하고 있음을 짚는 것.
          id: 'raised_ledger_question',
          description: '장부 기록의 증거력을 밤톨에게 환기함',
          triggerCondition:
        ...
        {
          // ID 유지, 내용 교체. 2차 서사: 차카의 "야미는 책을 많이 산다"는 악의 없는 발언을
          // 밤톨이 "여러 권 훔쳤을 것"으로 확대 해석한 게 오해의 핵심.
          // → 유저가 이 간극을 짚거나, 차카 발언이 훔침의 근거가 될 수 없음을 지적.
          // (기존 "사진 불완전성" 지적 경로도 하위 호환으로 유지)
          id: 'questioned_photo_evidence',
          description: '차카의 발언과 실제의 간극을 짚음',
          triggerCondition:
  ...
  npcDialogueContext: {
    ...
    yami: {
      ...
      stages: {
        dormant:
          '오늘 밤에 서점에 들러 예약해 둔 책을 픽업할 계획이다. 기대가 크지만 ' +
          '겉으로 티 내진 않는다. 독서 모임 장소로 밤톨 서점을 빌릴 수 있을지 ' +
          '속으로 고민 중이다. 유저가 먼저 이 얘기를 꺼내게 유도하는 정도로만 ' +
          '힌트를 흘리면 좋다. 예: "책 좋아해?" 같은 흐름에서 "사실 요즘 내가 ' +
          '재밌는 거 하나 준비하고 있어" 정도.',
        triggered:
          '어젯밤 서점에 들러 예약한 책을 픽업했다. 장부에 사인도 했다. 너에게는 ' +
          '평범한 밤이었다. 그런데 갑자기 밤톨 사장님이 찾아와 책을 훔쳐갔다며 ' +
          '너를 몰아세운다. 소문이 어디서 났는지 이해를 할 수가 없다.',
        quest_active:
    ...
    bamtol: {
      ...
      stages: {
        dormant:
          '오늘은 서점이 조금 한산했다. 야미가 예약한 책이 입고되어 곧 픽업하러 ' +
          '올 거라는 걸 알고 있다. 유저가 요즘 어떠냐고 물으면 "손님이 뜸하지만 ' +
          '책들이 착실히 들어온다"는 식의 담담한 대답. 서점 운영에 대한 자부심이 ' +
          '은근히 배어나는 톤.',
        triggered:
        ...
        quest_active:
          ...
          '유저가 직접 "장부 확인해봤냐"고 짚으면, 사실 그 말에 찔려 장부를 ' +
          '다시 확인했고 야미의 사인이 분명 있었다는 걸 알게 되었다. 그런데도 ' +
          '"사인만으론 못 믿겠다"며 자기 느낌을 앞세운다.',
        resolved:
